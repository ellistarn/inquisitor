...
func writeOverview(w io.Writer, mod *Module, packages []*Package, functions []*Function, types_ []*Type) {
	...
	fmt.Fprintln(w, "            packages referenced through fields, parameters, return types, and method bodies.")
	fmt.Fprintln(w, "            Basili et al. (1996) found CBO correlates with fault-proneness; practitioners")
	fmt.Fprintln(w, "            use CBO > 5 as a threshold. Reduce external type dependencies or split the type.")
	fmt.Fprintln(w)
